...
public interface IViviendaRepository extends JpaRepository<Vivienda, Integer>{
	...
	@Query("SELECT v FROM Vivienda v LEFT JOIN FETCH v.propietarioVivienda p where p.idPropietario = :id")
	List<Vivienda> BuscarViviendasConPropietarioId(@Param("id") int id);
}
